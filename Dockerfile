...
#
# `src.main` imports settings, which require LIVEKIT_* env vars. `.env.*` is not
# in the build context (.dockerignore), and `lk agent deploy` strips LIVEKIT_*
# from uploaded secrets (they are injected at runtime only). Defaults below are
# build-time placeholders so validation passes; override when building locally,
# e.g. `docker build --build-arg LIVEKIT_URL=wss://... ...`.
ARG LIVEKIT_URL
ARG LIVEKIT_API_KEY
ARG LIVEKIT_API_SECRET
ENV LIVEKIT_URL=${LIVEKIT_URL:-wss://build.local/placeholder} \
    LIVEKIT_API_KEY=${LIVEKIT_API_KEY:-build-placeholder} \
    LIVEKIT_API_SECRET=${LIVEKIT_API_SECRET:-build-placeholder}
